...
export class Matrix {
    constructor(tuples) {
        ...
        if (Array.isArray(tuples[0])) {
            this.cells = this.fromArrays(tuples);
        } else {
            this.cells = this.fromVectors(tuples);
        }
    }
    ...
    multiply(that) {
        return that.constructor.name === 'Matrix' ? this.multiplyMatrix(that) : this.multiplyTuple(that);
    }
