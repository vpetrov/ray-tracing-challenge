...
import { Shape, Ray, Sphere, Plane, Cube, Cylinder, Cone } from './geometry.mjs';
import { Point, Vector, Matrix, srt } from './tuple.mjs';
...
test('Intersecting a transformed group', () => {
    const group = new Group('group', Matrix.scaling(2, 2, 2));
...
test('Converting a point from world to object space', () => {
    const group1 = new Group('group1', Matrix.rotationY(Math.PI / 2));
    const group2 = new Group('group2', Matrix.scaling(2, 2, 2));
    group1.add(group2);
    const sphere = new Sphere(undefined, undefined, Matrix.translation(5, 0, 0));
    group2.add(sphere);
...
test('Converting a normal from object to world space', () => {
    const group1 = new Group('group1', Matrix.rotationY(Math.PI / 2));
    const group2 = new Group('group2', Matrix.scaling(1, 2, 3));
    group1.add(group2);
    const sphere = new Sphere(undefined, undefined, Matrix.translation(5, 0, 0));
    group2.add(sphere);
...
/*   A
     ...
     a1  B  a2
       ...
       b1  C
         ...
         c1  D
               */
test('group.child returns the right child by id or by name', () => {
    const groupA = new Group('groupA');
    const a1 = new Point();
    const a2 = new Point();
    const groupB = new Group('groupB');
    const b1 = new Vector();
    const groupC = new Group('groupC');
    const c1 = new Point();
    const groupD = new Group('groupD');

    groupA.add(a1, groupB, a2);
    groupB.add(b1, groupC);
    groupC.add(c1, groupD);

    expect(groupA.child('groupD').equals(groupD)).toBeTruthy();
    expect(groupC.child('groupD').equals(groupD)).toBeTruthy();
    expect(groupB.child(0).equals(b1)).toBeTruthy();
    expect(groupA.child(2).equals(a2)).toBeTruthy();
});

test('Empty groups can be cloned', () => {
    const group1 = new Group('A', Matrix.identity(4), true);
    const group2 = group1.clone();

    expect(group1.equals(group2)).toBeTruthy();
    expect(group2.id).toBeFalsy();

    group2.transform = Matrix.scaling(1, 2, 3);
    expect(group1.equals(group2)).toBeFalsy();
});

test('Groups with children can be cloned', () => {
    const group1 = new Group('A');
    group1.add(new Cube());
    group1.add(new Sphere());
    const group2 = group1.clone();

    expect(group1.equals(group2)).toBeTruthy();
    group2.childAt(0).transform = Matrix.rotationY(Math.PI / 2);

    expect(group1.equals(group2)).toBeFalsy();
});

test('Cloned groups preserve materials', () => {});
