import { eqf, cos, sin } from './math.mjs';
...
export class Matrix {
    ...
    multiplyMatrix(that) {
        ...
        for (let i = 0; i < this.size; ++i) {
            ...
            for (let j = 0; j < this.size; ++j) {
                row.push(this.row(i).multiply(that.column(j)).sum());
            }
            result.push(row);
        }
    ...
    multiplyTuple(tuple) {
        ...
        let result = [];
        for (let i = 0; i < this.size; ++i) {
            result.push(this.row(i).multiply(tuple).sum());
        }
        return new Tuple(...result);
    }
    ...
    static rotationX(radAngle) {
        const sinR = sin(radAngle);
        const cosR = cos(radAngle);
    ...
    static normalize(bounds) {
        const delta = bounds.maxPoint.minus(bounds.minPoint);
        return Matrix.scaling(1 / delta.x, 1 / delta.y, 1 / delta.z);
    }
}
