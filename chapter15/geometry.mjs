import { Point, Vector, Matrix, Color, srt } from './tuple.mjs';
import { Material, PointLight, Phong } from './shading.mjs';
...
export class Shape {
    ...
    get bounds() {
        throw Error(`Derived shape (${this.constructor.name}) does not implement required getter .bounds()!`);
    }
    ...
    // @param intersection only used by SmoothTriangle
    normal(worldPoint, intersection) {
        const localPoint = this.localPoint(worldPoint);
        const localNormal = this.localNormal(localPoint, intersection);
        return this.worldNormal(localNormal);
    }

    worldNormal(localNormal) {
        let result = this.transform.inverse().transpose().multiply(localNormal);
        result.w = 0;
        result = result.normalize();

        if (this.parent) {
            result = this.parent.worldNormal(result);
        }
    ...
    localPoint(worldPoint) {
        let point = this.parent ? this.parent.localPoint(worldPoint) : worldPoint;
    ...
    clone() {
        throw new Error(`Derived shape (${this.constructor.name}) does not implement required method .clone()`);
    }
}
...
export class Sphere extends Shape {
    ...
    clone() {
        return new Sphere(this.origin.clone(), this.radius, this.transform.clone(), this.material.clone());
    }
    ...
    get bounds() {
        // a sphere always extends from -1 to 1 on all axes
...
export class Plane extends Shape {
    ...
    clone() {
        return new Plane(this.transform.clone(), this.material.clone());
    }
...
export class BoundsCube extends Cube {
    constructor(objectBounds, transform, material) {
        super(transform, material);
        const minp = objectBounds.minPoint;
        const maxp = objectBounds.maxPoint;
        const xscale = maxp.x - minp.x;
        const yscale = maxp.y - minp.y;
        const zscale = maxp.z - minp.z;

        this.objectBounds = objectBounds;
        this.transform = srt(Matrix.scaling(xscale, yscale, zscale), transform);

        if (material) {
            this.material = material;
        } else {
            this.material = new Material(Color.RED);
            this.material.ambient = 0.1;
            this.material.diffuse = 0.1;
            this.material.specular = 0;
            this.material.shininess = 0;
            this.material.reflective = 0;
            this.material.refractive = 0.99;
...
export class Triangle extends Shape {
    constructor(p1, p2, p3, skipNormals) {
        super();
        this.p1 = p1;
        this.p2 = p2;
        this.p3 = p3;
        this.e1 = this.p2.minus(this.p1);
        this.e2 = this.p3.minus(this.p1);
        if (!skipNormals) {
            this.normalv = this.e2.cross(this.e1).normalize();
        }
    }
    ...
    localIntersect(localRay) {
        const dir_cross_e2 = localRay.direction.cross(this.e2);
        const determinant = this.e1.dot(dir_cross_e2);
        if (Math.abs(determinant) < EPSILON) {
            return [];
        }

        const f = 1.0 / determinant;
        const p1_to_origin = localRay.origin.minus(this.p1);
        ...
        const origin_cross_e1 = p1_to_origin.cross(this.e1);
        const v = f * localRay.direction.dot(origin_cross_e1);
        if (v < 0 || u + v > 1) {
            return [];
        }

        const t = f * this.e2.dot(origin_cross_e1);

        return [new Intersection(t, this, u, v)];
    }

    get bounds() {
        return new BoundingBox(this.p1, this.p2, this.p3);
    }

    equals(that) {
        return that != null &&
            that.constructor.name === this.constructor.name &&
            this.p1.equals(that.p1) &&
            this.p2.equals(that.p2) &&
            this.p3.equals(that.p3) &&
            this.normalv
            ? this.normalv.equals(that.normalv)
            : true;
    }

    clone() {
        return new Triangle(this.p1.clone(), this.p2.clone(), this.p3.clone(), !this.normalv);
    }
}
...
export class SmoothTriangle extends Triangle {
    ...
    localNormal(point, intersection) {
        const combinedN2 = this.n2.multiply(intersection.u);
        const combinedN3 = this.n3.multiply(intersection.v);
        const combinedN1 = this.n1.multiply(1 - intersection.u - intersection.v);
        return combinedN2.plus(combinedN3).plus(combinedN1);
    }

    equals(that) {
        return (
            that != null &&
            that instanceof SmoothTriangle &&
            this.p1.equals(that.p1) &&
            this.p2.equals(that.p2) &&
            this.p3.equals(that.p3) &&
            this.n1.equals(that.n1) &&
            this.n2.equals(that.n2) &&
            this.n3.equals(that.n3)
        );
    }
...
export class Intersection {
    ...
    info(ray, intersections) {
        ...
        let normalv = object.normal(point, this);
        let inside = false;
    ...
    static hit(intersections, forShadow) {
        ...
        for (let intersection of intersections) {
            if (forShadow && !intersection.object.castShadow) {
                continue;
            }
...
export class World {
    ...
    isShadowed(point) {
        ...
        const intersections = this.intersect(ray);
        const hit = Intersection.hit(intersections, true);
...
// an array of objects that implement the .equal() method
export class ObjectArray {
    ...
    checkObjects(objects) {
        if (!Array.isArray(objects)) {
            throw new Error("Argument 'objects' is not an array");
        }

        for (let i = 0; i < objects.length; i++) {
            this.checkObject(objects[i]);
        }
    }

    checkObject(object) {
        if (!object || typeof object.equals !== 'function') {
            throw new Error(`ObjectArray error: object (${object.constructor.name}) does not implement .equals()`);
        }
    }
    ...
    equals(that) {
        if (!that || !that.objects || !Array.isArray(that.objects) || this.objects.length !== that.objects.length) {
            return false;
        }
    ...
    apply(f) {
        ...
        for (let i = 0; i < this.objects.length; i++) {
            result.push(f(this.objects[i], i));
        }
    ...
    append(objectArray) {
        ...
        for (let i = 0; i < objectArray.length; i++) {
            this.push(objectArray.objects[i]);
        }
    }
}
