...
export class RayTracer {
    constructor(world, cameras, options) {
        ...
        this.options.format = options.format || 'ppm';
        this.options.prefix = options.prefix || 'screenshot_';
        ...
        if (progress.pixels > 0) {
            console.log(
                `[master] ${cameraId} progress: ${progress.percent}% (${progress.pixels}/${
                    progress.npixels
                }) (${pixelsPerSecond} px/s) (${progress.intersections} xs) (ETA: ${this.toETA(progress.eta)})`
            );
        }
    }
        ...
        switch (job.type) {
            case 'render': {
                ...
                if (this.cameraJobs[job.cameraId] <= 0) {
                    canvas.save(this.options.format, job.filename);
    ...
    render(cameraIds) {
        if (cluster.isMaster) {
            ...
            const allCameraIds = Object.keys(this.cameras);
            let renderCameraIds = [];
            // add all cameras that were autocreated
            for (const cameraId of cameraIds) {
                renderCameraIds = renderCameraIds.concat(
                    allCameraIds.filter((allCameraId) => allCameraId.includes(cameraId))
                );
            }

            const nworkers = os.cpus().length;
            this.createCanvases(renderCameraIds);
            this.scheduleJobs(renderCameraIds, nworkers);
    ...
    createCanvases(cameraIds) {
        for (const cameraId of cameraIds) {
            ...
            this.canvases[cameraId] = new Canvas(camera.hsize, camera.vsize, Color.BLACK);
        }
    }
        ...
        for (const cameraId in cameras) {
            ...
            for (let i = 0; i < frames; i++) {
                ...
                const camera = new Camera(
                    ...
                    cameraInfo.imageSize,
                    cameraInfo.fov,
                    View.transform(cameraInfo.from, cameraInfo.to, cameraInfo.up),
                    this.onProgress.bind(this)
                );

                result[cameraId + (i ? i : '')] = camera;
            }
        }
        return result;
    }
        ...
        for (const cameraId of cameraIds) {
            ...
            while (from < camera.vsize) {
                this.jobs.push({
                    workers: nworkers,
                    type: 'render',
                    filename: `${this.options.prefix}${cameraId}.${this.options.format}`,
                    cameraId: cameraId,
                    from: from,
                    to: to,
                });
